...
public class PBWInstallHandler implements InstallHandler {
    ...
    public void validateInstallation(InstallActivity installActivity, GBDevice device) {
        ...
        if (device.getType() != DeviceType.PEBBLE || !device.isConnected()) {
            installActivity.setInfoText(GBApplication.getContext ().getString (R.string.element_cannot_be_installed));
            installActivity.setInstallEnabled(false);
            return;
        }
    ...
    public void onStartInstall(GBDevice device) {
        if (mPBWReader == null || mPBWReader.isFirmware() || mPBWReader.isLanguage()) {
            return;
        }
