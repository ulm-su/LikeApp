...
public class GBDevice implements Parcelable {
    public static final String ACTION_DEVICE_CHANGED
            = "org.likeapp.likeapp.gbdevice.action.device_changed";
    public static final String ACTION_DEVICE_UPDATE = "org.likeapp.likeapp.device.action.device_update";
    ...
    public static final String EXTRA_UPDATE_PERCENT = "percent update";
    public static final String DEVINFO_HW_VER = "HW: ";
    ...
    private int mBatteryLife = -1;  // Время прошедшее от даты последней зарядки устройства, в часах
    ...
    private boolean waitingForUpdateFinish;
    ...
    private String getStateString(boolean simple) {
        switch (mState) {
            ...
            case INITIALIZED:
                if (waitingForUpdateFinish) {
                    return GBApplication.getContext().getString(R.string.waiting_for_update_finish);
                }
                if (simple) {
                    return GBApplication.getContext().getString(R.string.connected);
                }
    ...
    public boolean hasBatteryLevelForInstall ()
    {
        return mBatteryLevel > mBatteryThresholdPercent;
    }
    ...
    public List<ItemWithDetails> getDeviceInfos() {
        ...
        if (mSerialNumberString != null) {
            result.add(new GenericItem(DEVINFO_SN, mSerialNumberString));
        }
        if (mSerialNumberMD5 != null) {
            result.add(new GenericItem(DEVINFO_UID, GB.hexdump (mSerialNumberMD5, false)));
        }
    ...
    public void setWaitingForUpdateFinish (boolean waiting)
    {
        waitingForUpdateFinish = waiting;
    }

    public void setSerialNumber (byte[] sn)
    {
        ...
        mSerialNumberString = sb.toString ();

        try
        {
            MessageDigest digest = MessageDigest.getInstance ("MD5");
            digest.reset ();
            digest.update (mSerialNumberString.getBytes ());
            mSerialNumberMD5 = digest.digest ();
        }
