...
import org.likeapp.likeapp.database.DBOpenHelper;
...
import org.likeapp.likeapp.externalevents.BluetoothStateChangeReceiver;
import org.likeapp.likeapp.impl.GBDevice;
import org.likeapp.likeapp.impl.GBDeviceService;
...
import org.likeapp.likeapp.service.NotificationCollectorMonitorService;
...
import static org.likeapp.likeapp.model.DeviceType.AMAZFITCOR;
import static org.likeapp.likeapp.model.DeviceType.AMAZFITCOR2;
import static org.likeapp.likeapp.model.DeviceType.MIBAND;
import static org.likeapp.likeapp.model.DeviceType.MIBAND2;
import static org.likeapp.likeapp.model.DeviceType.MIBAND3;
import static org.likeapp.likeapp.model.DeviceType.fromKey;
...
public class GBApplication extends Application {
    ...
    private static final String TAG = "LikeApp-" + BuildConfig.VERSION_NAME;
    public static final String DATABASE_NAME = "LikeApp";
    ...
    public void onCreate() {
        ...
        // Если настроено "Запускать автоматически"
        if (gbPrefs.getAutoStart ())
        {
    ...
    public void updateLoggingToUlm ()
    {
        if (isLoggingToUlmEnabled ())
        {
            String id = prefs.getString ("log_to_ulm_id", "");
            if (id.length () < 4)
            {
                id = Integer.toHexString ((int) System.currentTimeMillis ()).toUpperCase ();
                sharedPrefs.edit ().putString ("log_to_ulm_id", id).apply ();
            }

            String name = TAG + " " + id;
            ClientLog.setOut (name);
            ClientLog.setErr (name);

            Log.setCheckLoggable (!prefs.getBoolean ("log_to_ulm_all", false));
        }
        else
        {
    ...
    public static boolean isLoggingToUlmEnabled() {
        return prefs.getBoolean("log_to_ulm", false);
    }
...
//        return prefs.getString("pref_key_theme", context.getString(R.string.pref_theme_value_dark)).equals(context.getString(R.string.pref_theme_value_dark));
    ...
    public String getNameAndVersion() {
        try {
            ...
            GB.log("Unable to determine LikeApp's name/version", GB.WARN, e);
            return "LikeApp";
        }
    }
}
