...
import org.likeapp.likeapp.service.btle.actions.NotifyAction;
import org.likeapp.likeapp.service.btle.actions.ReadAction;
import org.likeapp.likeapp.service.btle.actions.RequestMtuAction;
import org.likeapp.likeapp.service.btle.actions.WaitAction;
...
public class TransactionBuilder {
    ...
    public TransactionBuilder read(BluetoothGattCharacteristic characteristic) {
        if (characteristic == null) {
            LOG.trace ("Unable to read characteristic: null", new Exception ());
            return this;
        }
        LOG.trace ("<--- READ FROM: " + characteristic.getUuid ());
    ...
    public TransactionBuilder write(BluetoothGattCharacteristic characteristic, byte[] data) {
        if (characteristic == null) {
            LOG.trace ("Unable to write characteristic: null", new Exception ());
            return this;
        }
        LOG.trace ("---> WRITE:  " + characteristic.getUuid () + ": " + GB.hexdump (data, 0, data.length, true));
    ...
    public TransactionBuilder notify(BluetoothGattCharacteristic characteristic, boolean enable) {
        if (characteristic == null) {
            LOG.trace ("Unable to notify characteristic: null", new Exception ());
            return this;
        }
        LOG.trace ("---> NOTIFY: " + characteristic.getUuid () + " = " + enable);
